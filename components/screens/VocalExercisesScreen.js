...
const VocalExercisesScreen = ({ navigation, route }) => {
  ...
  const loadUserProgress = async (userId) => {
    try {
      ...
      const response = await fetch(`http://31.220.50.7:3000/vocal-exercises/progress/${userId}`);
  ...
  const saveProgressToServer = async (challengeId, weekNumber, status) => {
    ...
    try {
      ...
      const response = await fetch(`http://31.220.50.7:3000/vocal-exercises/progress`, {
        method: 'POST',
        headers: {
