...
const SpeechChallengesScreen = ({ navigation, route }) => {
  ...
  const loadChallenges = async (userId) => {
    try {
      ...
      const response = await fetch(`http://31.220.50.7:3000/speech-challenges/${userId}`);
  ...
  const assignDailyChallenge = async (userId) => {
    try {
      ...
      const response = await fetch(`http://31.220.50.7:3000/speech-challenges/assign-daily`, {
        method: 'POST',
        headers: {
  ...
  const updateChallengeStatus = async (challengeId, status) => {
    try {
      ...
      const response = await fetch(`https://31.220.50.7:3000/speech-challenges/${challengeId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
