...
const ProgressScreen = ({ navigation, route }) => {
  ...
  const loadAllProgress = async (userId) => {
    try {
      ...
      const exercisesResponse = await fetch(`http://31.220.50.7:3000/progress/stats/${userId}`);
      ...
      const vocalResponse = await fetch(`http://31.220.50.7:3000/vocal-exercises/progress/${userId}`);
      if (!vocalResponse.ok) {
      ...
      const vocalStatsResponse = await fetch(`http://31.220.50.7:3000/vocal-exercises/stats/${userId}`);
      const vocalStatsData = await vocalStatsResponse.json();
      console.log('Estadísticas vocales:', vocalStatsData);

      // Cargar retos de habla
      const speechChallengesResponse = await fetch(`http://31.220.50.7:3000/speech-challenges/${userId}`);
