...
const ProgressScreen = ({ navigation, route }) => {
  ...
  const loadAllProgress = async (userId) => {
    try {
      ...
      const exercisesResponse = await fetch(`http://[2a02:4780:10:40ef::1]:3001/progress/stats/${userId}`);
      ...
      const vocalResponse = await fetch(`http://[2a02:4780:10:40ef::1]:3001/vocal-exercises/progress/${userId}`);
      if (!vocalResponse.ok) {
      ...
      const vocalStatsResponse = await fetch(`http://[2a02:4780:10:40ef::1]:3001/vocal-exercises/stats/${userId}`);
      const vocalStatsData = await vocalStatsResponse.json();
      console.log('Estadísticas vocales:', vocalStatsData);

      // Cargar retos de habla
      const speechChallengesResponse = await fetch(`http://[2a02:4780:10:40ef::1]:3001/speech-challenges/${userId}`);
